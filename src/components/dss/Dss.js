...
const Dss = () => {
  ...
  const [totalDss, setTotalDss] = useState('');
  ...
  const preferenceTableName = 'dssTablePreferences';
  const [dssPageDetails, setDssPageDetails] = useState('');
  ...
  const fetchDss = async (page = currentPage, pageSize = itemsPerPage) => {
    setIsLoading(true);
    const setItemsPerPage = pageSize;
    const apiClient = new ApiClient({
      url: '/api/distribution-transformers',
      headers: {
        'disco': 'root',
      },
      params: {
      ...
      onSuccess: (data) => {
        setDssPageDetails(data.data);
        setDss(data.data.data);
  ...
  return (
    <div className="flex m-6 bg-white flex-col">
      ...
      <div className="p-4 flex w-full h-20 justify-content justify-between">
        <p className='w-full text-xl font-semibold text-mygard-1'>DSS List</p>
